...
final bookDetailsProvider = StateNotifierProvider.family<BookDetailsNotifier, BookDetailsState, int>((ref, bookId) {
  final bookRepository = ref.watch(bookRepositoryProvider);
...
final bookByIdProvider = FutureProvider.family<BookModel?, int>((ref, bookId) async {
  final bookRepository = ref.watch(bookRepositoryProvider);
...
class ReviewsNotifier extends StateNotifier<ReviewsState> {
  ...
  ReviewsNotifier(this._bookRepository, this.bookId) : super(const ReviewsState());
  ...
  Future<void> deleteReview(int reviewId) async {
    try {
      ...
      if (success) {
        state = state.copyWith(
          reviews: state.reviews.where((review) => review.id != reviewId).toList(),
        );
      }
    } catch (e) {
      state = state.copyWith(error: e.toString());
...
final reviewsProvider = StateNotifierProvider.family<ReviewsNotifier, ReviewsState, int>((ref, bookId) {
  final bookRepository = ref.watch(bookRepositoryProvider);
...
final bookReviewsProvider = FutureProvider.family<List<Review>, int>((ref, bookId) async {
  final bookRepository = ref.watch(bookRepositoryProvider);
