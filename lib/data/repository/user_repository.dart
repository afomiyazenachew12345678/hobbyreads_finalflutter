...
class UserRepository {
  ...
  Future<Map<String, dynamic>> updateUserSettings(String userId, Map<String, dynamic> settings) async {
    try {
      final response = await _apiService.put('/users/$userId/settings', body: settings);
      return response;
    } catch (e) {
  ...
  // User Activity
  Future<List<Map<String, dynamic>>> getUserActivity(String userId, {int page = 1, int limit = 20}) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> getUserReadingHistory(String userId, {int page = 1, int limit = 20}) async {
    try {
      final response = await _apiService.get(
  ...
  Future<bool> verifyUserEmail(String token) async {
    try {
      final response = await _apiService.post('/users/verify-email', body: {'token': token});
  ...
  Future<bool> requestEmailVerification(String email) async {
    try {
      final response = await _apiService.post('/users/request-verification', body: {'email': email});
  ...
  Future<Map<String, dynamic>> updateUserPreferences(String userId, Map<String, dynamic> preferences) async {
    try {
      final response = await _apiService.put('/users/$userId/preferences', body: preferences);
      return response;
    } catch (e) {
  ...
  Future<Map<String, dynamic>> updateNotificationSettings(String userId, Map<String, dynamic> settings) async {
    try {
      final response = await _apiService.put('/users/$userId/notifications', body: settings);
      return response;
    } catch (e) {
  ...
  // User Privacy
  Future<Map<String, dynamic>> updatePrivacySettings(String userId, Map<String, dynamic> settings) async {
    try {
      final response = await _apiService.put('/users/$userId/privacy', body: settings);
      return response;
    } catch (e) {
